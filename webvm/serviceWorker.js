async function handleFetch(request) {
	...
	if (!r) {
		// Try backup fetch or return custom error response
		try {
			r = await fetch(request);
		} catch (err) {
			return new Response("Network error: " + (e ? e.message : ""), { status: 480 });
		}
	}
	if (r.status === 0) {
		return r;
...
async function doRegister() {
	try {
		const scriptUrl = (window.document.currentScript && window.document.currentScript.src) || '/webvm/serviceWorker.js';
