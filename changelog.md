...
- **sshd PermitRootLogin override via hookscript.** The distros' default `PermitRootLogin prohibit-password` (compiled into openssh-server) was rejecting root password logins to LXC containers even though sshd was listening on `:22`. `createLxcContainer` now registers `hookscript: local:snippets/mitch-sshd-bootstrap.sh` on every new container; the host-side hookscript runs during pre-start and writes `/etc/ssh/sshd_config.d/99-mitch.conf` with `PermitRootLogin yes`. Idempotent. Repo copy at `tools/proxmox-hookscript-mitch-sshd-bootstrap.sh`; install on tartarus with `install -m 0755 ... /var/lib/vz/snippets/mitch-sshd-bootstrap.sh`.
