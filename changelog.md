...
- **PVE fetch timeouts + non-JSON response handling.** `createLxcContainer`, `getUserVmStatus`, `getExistingVmids`, and the QEMU agent call now go through `fetchWithDeadline(...)` (15s / 5s / 5s / 3s respectively). Previously they used a bare `fetch()` — if the Proxmox API was unreachable (e.g. dev container pointed at `https://localhost:8006` that wasn't port-forwarded) the request hung and the browser eventually saw an empty body, surfacing as `JSON.parse: unexpected end of data`. Now those paths fail fast with a clear error message (`Proxmox unreachable (pct create timed out)` / `Proxmox returned non-JSON (502): …`).
