...
`;
  ...
  async cloneDesktop({ templateVmid, vmid, hostname, cpuCores = 6, memoryMb = 16384, diskGb = 40, desktopUsername, desktopPassword }) {
    ...
    memoryMb = Math.max(2048, Math.min(Math.round(Number(memoryMb) || 16384), 32768));
    ...
    void this.enableFriendlyDesktopLogin(vmid, login.username).then(() => {
