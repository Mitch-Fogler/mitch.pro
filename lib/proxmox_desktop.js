...
`;
  ...
  async cloneDesktop({ templateVmid, vmid, hostname, cpuCores = 6, memoryMb = 65536, diskGb = 40, desktopUsername, desktopPassword }) {
    ...
    cpuCores = Math.max(2, Math.min(Math.round(Number(cpuCores) || 6), 16));
    memoryMb = Math.max(2048, Math.min(Math.round(Number(memoryMb) || 65536), 65536));
    ...
    await this.request('PUT', `/nodes/${encodeURIComponent(this.node)}/qemu/${vmid}/config`, {
      ...
      balloon: Math.min(4096, Math.max(2048, Math.floor(memoryMb / 4))), agent: 1,
  ...
  async unlockDesktopSession(vmid) {
    vmid = this.assertVmid(vmid);
    try {
      await this.guestExec(vmid, ['/bin/sh', '-c', 'loginctl unlock-sessions >/dev/null 2>&1 || true'], '', 10_000);
      return { success: true };
    } catch (err) {
      console.warn(`[proxmox] unlockDesktopSession for VM ${vmid}:`, err?.message || err);
      return { success: false, error: err?.message || 'Could not unlock session' };
    }
  }
  ...
  validateDesktopLogin(username, password) {
    ...
    if (!/^[a-z][a-z0-9_\-]{1,31}$/.test(cleanUsername) || ['root', 'daemon', 'nobody', 'ubuntu'].includes(cleanUsername) || cleanPassword.length < 8 || cleanPassword.length > 128 || /[\r\n\0]/.test(cleanPassword)) {
      throw new ProxmoxServiceError('INVALID_DESKTOP_LOGIN', 'Choose a desktop username and a password of 8 to 128 characters.', 400);
    }
