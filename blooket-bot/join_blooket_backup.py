...
def preprocess_cheat_js(js_code):
    ...
    pattern = r'Object\.values\(\s*function\s+e\s*\(\s*t\s*=\s*document\.querySelector\(\s*["\']body\s*>\s*div["\']\s*\)\s*\)\s*\{\s*return\s+Object\.values\(\s*t\s*\)\[1\]\?\.children\?\.\[0\]\?\._owner\.stateNode\?t:e\(\s*t\.querySelector\(\s*["\']:scope\s*>\s*div["\']\s*\)\s*\)\s*\}\s*\(\s*\)\s*\)\[1\]\.children\[0\]\._owner(?:\[["\']stateNode["\']\]|\.stateNode)'
    js_code = re.sub(pattern, robust_lookup, js_code)
