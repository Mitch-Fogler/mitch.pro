(async () => {
    ...
    const opponent = prompt("Who would you like to gift crypto to? (Case sensitive)");
    if (!opponent) return;

    const amount = parseInt(prompt("How much crypto would you like to gift?")) || 0;

    e.safe = true;
    // Format is opponent:crypto_amount
    // The host interprets this to add/transfer crypto to the target player.
    await e.props.liveGameController.setVal({
        path: `c/${e.props.client.name}/tat`,
        val: `${opponent}:${amount}`
    });

    alert(`Gifted ${amount} crypto to ${opponent}!`);
})();
